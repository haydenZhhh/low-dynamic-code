<template>
    <el-form :rules="rules" ref="stackForm" :model="stackConfig" label-width="auto" label-position="top" size="large">
        ...
        <el-form-item label="下拉数据">
            <div class="addPanenl" ><el-button @click="addData()" size="large" type="primary"  link :icon="Plus" /></div>
            ...
            <div class="dataListMain">
                <el-row v-for="(iteam,index) in stackConfig.selectData" :key="index">
                    <el-col :span="10">
                        <el-input v-model="iteam.label" @input="changeInput(index)" placeholder="label" />
                     </el-col>
                    <el-col :span="10">
                        <el-input v-model="iteam.value" placeholder="value" /> 
                    </el-col>
                    <el-col :span="4" style="text-align: center;display: flex;justify-content: center;align-items: center;">
                        <el-icon @click="deleteIteam(index)" style="font-size: 20px;color: red;"><CircleClose   /></el-icon>
                    </el-col>
                </el-row>
            </div>
        </el-form-item>
        <el-form-item label="是否必填">
            <el-switch v-model="stackConfig.isMustValue" size="large" active-text="是" inactive-text="否" @change="(val) => {
...
const deleteIteam = (vidx)=>{
    stackConfig.selectData = stackConfig.selectData.filter((val,idx)=>idx!==vidx)
}
